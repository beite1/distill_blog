...
Last year I helped deliver a project for the IFC and members of the Pacific Island Investment Forum (PIIF). The PIIF is an association of funds across the Pacific, including provident, superannuation, sovereign and trust funds. Examples include the Vanuatu National Provident Fund (VNPF) and the Fiji National Provident Fund (FNPF). In many Pacific Island nations they represent the largest domestic asset owner and investor. The project got me interested in understanding their role further, so I did what feels natural and started looking for data. Also, this is the Island Economist blog so lets talk about some island institutions!

The data I eventually found was in the financial accounts. As far as I can tell no has ever attempted to extract this data and undertake analysis. Which doesn't surprise me, the data is held in pdfs and in some cases scanned pdfs. The effort required to extract the data was substantial and mostly manual. If any of the following analysis is incorrect its likely that I mis-typed a number. Of the 16 PIIF members this analysis only covers 9 where I was able to compile enough data. 
...
The figure below shows the arithmetic mean annual real return and standard deviation for each of the funds. Standard deviation is a measure of volatility and by extension risk. There seems to be three general groupings. Two high performers (Cook Islands and Solomon Islands), a group in the middle and two low performers (Vanuatu and Tonga). 
...
plot_ly(marker = list(size = 20)) %>% 
  ...
  layout(showlegend = FALSE,
         ...
         yaxis = list(title = "Arithmetic mean real returns",range = c(-1, 10))) %>%
